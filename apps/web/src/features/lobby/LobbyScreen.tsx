  ...
  return (
    <main className="lobby-screen">
      <header className="operation-bar">
        <button className="operation-brand" type="button" onClick={onBack}><span>V</span><strong>5일 뒤 마왕</strong></button>
        <div><span className="status-dot" /> LOBBY NETWORK</div>
        <div className="operator-id"><small>{viewer.accountType === "guest" ? "GUEST" : "OPERATIVE"}</small><strong>{viewer.displayName}</strong></div>
      </header>

      <div className="lobby-workspace">
        <section className="chat-column" aria-labelledby="chat-title">
          <div className="workspace-heading"><span>01</span><div><h2 id="chat-title">파티 채널</h2><small>{snapshot ? snapshot.roomName : "방에 참가하면 열립니다"}</small></div></div>
        ...
        <section className="room-column" aria-labelledby="room-title">
          <div className="room-title-row"><div className="workspace-heading"><span>02</span><div><h1 id="room-title">공개 원정대</h1><small>빈자리가 있는 작전실에 참가하세요</small></div></div><button className="create-room-button" type="button" disabled={!!snapshot} onClick={() => setCreateOpen(true)}>+ 방 만들기</button></div>
          ...
          {!snapshot ? <div className="room-join-strip"><div><small>SELECTED ROOM</small><strong>{selectedRoom?.roomName ?? "원정대를 선택하세요"}</strong></div><button type="button" disabled={!selectedRoom || selectedRoom.clients >= 3 || selectedRoom.phase !== "waiting" || busy} onClick={() => selectedRoom && void onJoin(selectedRoom.roomId)}>참가하기 <span>→</span></button></div> : null}
        </section>

        <aside className="party-column" aria-labelledby="party-title">
          <div className="workspace-heading"><span>03</span><div><h2 id="party-title">현재 파티</h2><small>{snapshot ? `${snapshot.players.length} / 3 OPERATIVES` : "NO ACTIVE PARTY"}</small></div></div>
...
function CreateRoomDialog({ busy, onClose, onCreate }: { busy: boolean; onClose: () => void; onCreate: (input: { roomName: string; sessionMode: "prototype" | "full"; difficulty: "easy" | "normal" | "hard" }) => Promise<void> }) {
  ...
  return <div className="create-backdrop" role="dialog" aria-modal="true" aria-labelledby="create-title"><form className="create-dialog" onSubmit={(event) => { event.preventDefault(); void onCreate({ roomName: roomName.trim(), sessionMode, difficulty }); }}><span>NEW EXPEDITION</span><h2 id="create-title">원정대 편성</h2><label>방 이름<input value={roomName} maxLength={24} minLength={2} required onChange={(event) => setRoomName(event.target.value)} placeholder="예: 새벽의 원정대" /></label><fieldset><legend>세션</legend><button type="button" className={sessionMode === "prototype" ? "active" : ""} onClick={() => setSessionMode("prototype")}>프로토타입 <small>8분</small></button><button type="button" className={sessionMode === "full" ? "active" : ""} onClick={() => setSessionMode("full")}>정식 흐름 <small>25분</small></button></fieldset><fieldset><legend>난이도</legend>{(["easy", "normal", "hard"] as const).map((value) => <button key={value} type="button" className={difficulty === value ? "active" : ""} onClick={() => setDifficulty(value)}>{difficultyLabel(value)}</button>)}</fieldset><div><button type="button" onClick={onClose}>취소</button><button type="submit" disabled={busy || roomName.trim().length < 2}>방 만들기</button></div></form></div>;
}

function difficultyLabel(value: "easy" | "normal" | "hard") { return value === "easy" ? "이지" : value === "normal" ? "노말" : "하드"; }
