  ...
  return <main className={`character-select-screen ${launching ? "is-launching" : ""}`}>
    ...
    <section className="class-slashes" aria-label="캐릭터 클래스 선택">
      {DISPLAY_ORDER.map((classId) => {
        ...
        return <button type="button" key={classId} className={`slash slash--${classId} ${selected ? "is-selected" : ""}`} aria-pressed={selected} onClick={() => onSelect(selected ? null : classId)} style={{ "--class-accent": definition.cssColor } as React.CSSProperties}>
