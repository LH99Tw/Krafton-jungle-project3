...
- 서버 실행: 로그인→로비→방 생성·참가→게임 시작의 모든 인게임 플레이가 서버 snapshot과 world frame만 상태 원본으로 사용한다. 연결 실패 시 브라우저 시뮬레이션으로 전환하지 않고 시작 오류를 표시한다.
- 에디터 코어: 개발 환경의 맵 편집기 플레이 테스트만 로컬 `GameCore`를 고정 틱으로 실행해 편집 맵을 검증한다.

두 실행은 `RoomGameScene`의 렌더링·입력·snapshot 변환을 공유한다. 실제 게임 규칙은 서버의 `GameCore`, 편집 맵 검증 규칙은 `editor-core`가 담당하며 브라우저 전용 전투·진행·절차형 맵 규칙은 두지 않는다.
...
| 채널 | 주기 | 내용 |
|---|---:|---|
| GameCore | 60Hz | 이동, 충돌, 전투, AI, 페이즈, 성장·보상 |
...
- 특수 방 규칙: 장비 보급·신전·함정·체크포인트·제단 상태와 상호작용
...
자동 테스트는 코어 결정론, 충돌, 시야, AI 경로, 장비 보급, 256 침략자 부하를 포함한다. 서버 테스트는 프로토콜, AOI, 재접속, 결과 저장, scheduler rollout을 포함한다.
